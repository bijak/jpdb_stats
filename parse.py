        ...
        for entry in (reviews_json["cards_vocabulary_jp_en"] + reviews_json["cards_kanji_char_keyword"] + reviews_json["cards_kanji_keyword_char"]):
            ...
            for review in entry['reviews']:
                if review['grade'] != 'abandoned':
                    rep.append(datetime.utcfromtimestamp(review['timestamp']).astimezone(pytz.timezone(timezone)))
...
def parse_entry(entry, timezone):
    ...
    for review in entry['reviews']:
        ...
        if previous is None:
            history.append([datetime.utcfromtimestamp(review['timestamp']).date(), 0, 0, 1, 0, isKnown])
        elif (previous['grade'] == 'abandoned') and review['grade'] != 'abandoned':
            history.append([datetime.utcfromtimestamp(review['timestamp']).date(), 0, 0, 1, 0, False])
        elif (review['grade'] == 'abandoned'):
            history.append([datetime.utcfromtimestamp(review['timestamp']).date(), 0, 0, 0, 1, False])
            ...
            history.append([datetime.utcfromtimestamp(review['timestamp']).astimezone(pytz.timezone(timezone)).date(), is_fail(review['grade']), is_successful(review['grade']), 0, 0, isKnown])
        previous = review
        ...
        if is_successful(review['grade']):
            ...
            if not everKnown:
                time_to_learn += 1 
    return history, [[entry.get('spelling') or ('Kanji: ' + entry['character']), len(entry['reviews']), time_to_learn, relapses, abandoned]]

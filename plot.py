...
def gen_cum_daily_plot(dataframe, type, unit, rolling=False, template="plotly_dark"):
    ...
    if rolling:
        ...
        f_daily.add_trace(
            go.Scatter(
                name="Rolling Average", x=rolling_df.index, y=rolling_df["Count"]
            )
        )
...
def gen_retention_plot(dataframe, type, template="plotly_dark"):
    ...
    dataframe = dataframe.reindex(idx, fill_value=0)
    dataframe["Retention"] = (
        100 * dataframe["Passed"] / (dataframe["Passed"] + dataframe["Failed"])
    )
    window_df = dataframe.rolling(7).sum()
    dataframe["Rolling Average"] = (
        100 * window_df["Passed"] / (window_df["Passed"] + window_df["Failed"])
    )
    f_retention = dataframe.plot(y="Retention", kind="bar")
    f_retention.add_trace(
        go.Scatter(
            name="Rolling Average", x=dataframe.index, y=dataframe["Rolling Average"]
        )
    )
...
def plot_reps(rep, template="plotly_dark"):
    ...
    reviews_minute["Count"] = reviews_minute["Count"].where(
        reviews_minute["Count"] == 0, 1
    )
    reviews_minute = reviews_minute.groupby(pd.Grouper(freq="D")).sum()

    fm_rc, fm_rd = gen_cum_daily_plot(
        reviews_minute, "Time Spent", "time (minutes)", rolling=True
    )
...
def plot_history(history, template="plotly_dark"):
    history_df = (
        pd.DataFrame(
            history, columns=["Date", "Failed", "Passed", "New", "Abandoned", "Known"]
        )
    ).drop(["Known"], axis=1)
    ...
    history_df = history_df.cumsum()
    f_history_cum = history_df.plot(kind="bar", color_discrete_sequence=colors)
    f_history_cum.update_layout(
        title="Cards (Cum.)",
        yaxis_title="Card Count",
        xaxis_title="Date",
        template=template,
    )
    return f_history, f_history_cum
...
def plot_retention(history, template="plotly_dark"):
    ...
    retention_known_df = (
        retention_df[retention_df["Known"] == True].groupby("Date").sum()
    )
    retention_learning_df = (
        retention_df[retention_df["Known"] == False].groupby("Date").sum()
    )

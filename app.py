...
default_fig = {}
default_df = pd.DataFrame(columns=["Word", "Total Reviews", "Time to Learn", "No. Relapses", "Abandoned"])
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody([
                    ...
                    dcc.Upload(
                        id="upload-data",
                        children=html.Div(["This should be the reviews.json downloaded from the JPDB Settings page. Drag and Drop or ", html.A("Select File")]),
                        style={
                            "width": "100%",
                            "height": "60px",
                            "lineHeight": "60px",
...
@app.callback(
    [Output("new_cum", "figure"),Output("new_daily", "figure"),Output("rev_cum", "figure"),Output("rev_daily", "figure"),Output("time_cum", "figure"),\
        Output("time_daily", "figure"),Output("overall", "figure"),Output("datatable-struggles","data"),Output("ret_known","figure"),Output("ret_learning","figure")],
    [Input("upload-data", "contents"), Input("upload-data", "filename"), Input("timezone", "data")],
)
def update_graph(contents, filename, timezone):
    f_nc = default_fig; f_nd = default_fig; f_rc = default_fig; f_rd = default_fig; fm_rc = default_fig; fm_rd = default_fig
    f_overall = default_fig; datatable = default_table; f_known = default_fig; f_learn = default_fig
    if (filename is not None):
        ...
        f_overall = plot.plot_history(history)
        f_known, f_learn = plot.plot_retention(history)
        datatable = pd.DataFrame(struggles, columns=["Word", "Total Reviews", "Time to Learn", "No. Relapses", "Abandoned"]).to_dict('records')

    return [f_nc, f_nd, f_rc, f_rd, fm_rc, fm_rd, f_overall, datatable, f_known, f_learn]

@app.callback(
    [Output("new_cum", "style"),Output("new_daily", "style"),Output("rev_cum", "style"),Output("rev_daily", "style"),Output("time_cum", "style"),\
        Output("time_daily", "style"),Output("overall", "style"),Output("ret_known","style"),Output("ret_learning","style")],
...
def update_timezone(value):
    dash.callback_context.response.set_cookie('jpdb-stats-tz', value)
    return value
